...
class ModelSimulation:
    ...
    def structure_max_drift_per_floor(self, **kwargs):
        ...
        for loc, df in enumerate(displacements[:2]):
            for i in range(self.stories):
                ...
                compute_drifts = [self._computeDriftBetweenNodes(df[current_node], df[following_node], self.heights[i],loc)
    ...
    def loadModelInfo(self):
        ...
        self.model_info = ModelInfo(sim_type=self._sim_type)
        ...
        print('Computing model information...')
    ...
    #NOTE: We have to compute the max value? or the value in the moment of the max drift ?
...
class ModelInfo:
    ...
    def __init__(self, sim_type:int = 1):
...
class Plotting:
    ...
    def __init__(self, database: ModelSimulation, path:str):
        ...
        self.sprectums_title     = f'Story Accelerations Spectrum Plot | {self.sim_type} |  {self.mag[:3]} | {self.rup_type} | Station {self.station}'
    ...
    def plotModelDrift(self, max_corner_x: list, max_center_x: list, max_corner_y: list, max_center_y:list ):
        ...
        ax.set_yticks(range(1, self.stories + 1))
    ...
    def plotStoriesSpectrums(self, ModelSimulation: ModelSimulation, dir_:str, accel_df:pd.DataFrame, stories_df:pd.DataFrame,
                             stories_lst:list[int], T:np.ndarray):
        # Input params
    ...
    def plotBaseSpectrum(self, ModelSimulation: ModelSimulation, accel_df:pd.DataFrame, stories_df:pd.DataFrame,
        ...
        for i, mode in enumerate(spectrum_modes):
            ax.axvline(x=mode, linestyle='--', label=f'Mode{i} = {mode}')
