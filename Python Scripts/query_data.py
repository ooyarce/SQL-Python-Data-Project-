...
class ModelSimulation:
    ...
    def model_structure_perfomance(self, **kwargs):
        ...
        def insert_story_nodes():
            ...
            #StoryNodesDataFrame = self.paralelize_serialization(self.story_nodes_df.iloc[8:])
            return StoryNodesDataFrame

        with ThreadPoolExecutor(max_workers=6) as executor:
            future_to_name = {
                executor.submit(insert_base_shear): "BaseShear",
                executor.submit(insert_max_base_shear): "MaxBaseShear",
                executor.submit(insert_abs_accelerations): "AbsAccelerations",
                executor.submit(insert_relative_displacements): "RelativeDisplacements",
                executor.submit(insert_max_drift_per_floor): "MaxDriftPerFloor",
                executor.submit(insert_story_accelerations): "StoryAccelerations",
                executor.submit(insert_story_nodes): "StoryNodesDataFrame",
                            }
        ...
        values = (
            results.get('BaseShear'),
            results.get('AbsAccelerations'),
            results.get('RelativeDisplacements'),
            results.get('MaxBaseShear'),
            results.get('MaxDriftPerFloor'),
            results.get('StoryAccelerations'),
            results.get('StoryNodesDataFrame'),
            comments)
        try:
            cursor.execute(insert_query, values) # type: ignore
